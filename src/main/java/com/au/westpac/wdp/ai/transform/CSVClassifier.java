...
public class CSVClassifier {
    ...
    private static Logger log = LoggerFactory.getLogger(CSVClassifier.class);

    private static Map<Integer,String> classifiers = readEnumCSV("/input/classifiers.csv");
    ...
    public static void main(String[] args){
        ...
        try {
            ...
            DataSet testData = readCSVDataset("/input/testlogs.csv",
            ...
            log.info("normalizer:"+normalizer.toString());
            log.info("trainingData:"+trainingData.toString());
    ...
    public static Map<Integer,Map<String,Object>> makeAnimalsForTesting(DataSet testData){
        ...
        for (int i = 0; i < rows ; i++) {
            try {
                ...
                //set the attributes
                records.put("noOfStepsExecuted", slice.getInt(0));
                records.put("noOfErrorsOccurred", slice.getInt(1));
                records.put("TotalExecutionTime", slice.getInt(2));
                records.put("TotalProcessingTime", slice.getInt(3));
                animals.put(i, records);
            }catch(Exception e){
                log.error("error",e);
            }
        }
